...
REG_TELEPHONE = r"\b(?:(\+\d\d))?(?:\d.? ?){9}\b"
# REG_BANK = r"(?:(?<=^)|(?<=\s)|(?<=:))([A-Z]{2}[0-9]{2} ?)?([0-9]{4} ?){2}[0-9]{2} ?[0-9]{10}\b"
REG_BANK = r"\b[A-Z]{2}[0-9]{2} ?([0-9]{4} ?){5}\b"
REG_BANK2 = r"(?:(?<=^)|(?<=\s)|(?<=:))[A-Z]{4} ?([A-Z]{2} ?){2}[0-9]{3}\b"
#OLD_REG_ID = r"[0-9]{8}[a-zA-Z]{1}|[a-zA-Z]{1}[0-9]{8}|[a-zA-Z]{3}[0-9]{6}"
DNI = r"(\d ?){8,10} ?-?[a-zA-Z]" + r"|" + r"\d{2}(\s|\.)?\d{3}(\s|\.)?\d{3}(\s|\.)?[a-zA-Z]{1}"
NIE = r"[a-zA-Z] ?(\d ?){6,8}[a-zA-Z]"
CATASTRO = r"\d{7} ?[A-Z]{2} ?[0-9]{4} ?[A-Z] ?\d{4} ?[A-Z]{2}" + r"|" + r"\d{2} ?\{3} ?[A-Z] ?\d{3} ?\d{5} ?\d{4} ?[A-Z]{2}" 
LICENCIA_OBRES = r"\d{2}(\s|-)\d{4}[A-Z]{2}\d{5}" 
NIF = r"[a-zA-Z] ?(\d ?){7} ?(\d|[a-zA-Z])"
NUM_ASENTAMENT = r"\d{10} ?-?\d ?-?\d{4} ?-?\d{7} ?-?\d" 
NEW_REG_ID = r"\b\(?(" + DNI + r"|" + NIE + r"|" + CATASTRO + r"|" + LICENCIA_OBRES + r"|" + NIF + r"|" + NUM_ASENTAMENT + r")\)?\b"
...
# REG_ID_OLD = r"\b(\d{2}(\s|\.)?\d{3}(\s|\.)?\d{3}(\s|\.)?[a-zA-Z]{1}|[a-zA-Z]{3}[0-9]{6}|[0-9]{2} ?[0-9]{8})\b" # NIE, DNI, CIF
